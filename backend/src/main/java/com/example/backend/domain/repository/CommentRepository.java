...
public interface CommentRepository extends JpaRepository<Comment, Long> {
    ...
    @Query("SELECT c FROM Comment c WHERE c.story.id = :storyId AND c.status = :status AND c.isActive = true ORDER BY c.id DESC")
    ...
    @Query("SELECT c FROM Comment c WHERE c.status = :status AND c.isActive = true ORDER BY c.id DESC")
    Page<Comment> findByStatus(@Param("status") CommentStatus status, Pageable pageable);
    
    @Query("SELECT c FROM Comment c JOIN c.story s WHERE s.user.id = :authorId AND c.isActive = true ORDER BY c.id DESC")
    Page<Comment> findByAuthorId(@Param("authorId") Long authorId, Pageable pageable);
    
    @Query("SELECT c FROM Comment c JOIN c.story s WHERE s.user.id = :authorId AND c.story.id = :storyId AND c.isActive = true ORDER BY c.id DESC")
